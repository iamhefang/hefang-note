...
export default function NoteTree({ search }: NoteTreeProps) {
  ...
  const onListRightClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      const itemId = (
        pathes.find((path) => path instanceof HTMLDivElement && path.getAttribute("data-id")) as HTMLDivElement
  ...
  const onDragOver = useCallback((e: React.DragEvent<HTMLDivElement>) => {
    if ((e.target as HTMLDivElement)?.parentElement?.getAttribute("data-type") === "note-tree") {
      e.stopPropagation()
      e.preventDefault()
    }
  }, [])
  ...
  const onDrop = useCallback(
    (e: React.DragEvent<HTMLDivElement>) => {
      ...
      dispatch(moveNote({ targetId: undefined, sourceId }))
    },
    [dispatch],
  )
