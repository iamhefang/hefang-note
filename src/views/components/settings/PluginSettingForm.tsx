...
export default function PluginSettingForm({ plugin }: PluginSettingFormProps) {
    ...
    const onValuesChange = useCallback(
        (changedValues: Partial<Settings>, values: Settings) => {
            console.log("插件配置变化", changedValues)
    ...
    const formItems = useMemo(() => {
        ...
        return Object.fromEntries(
            Object.entries(plugin.settings.items)
                .map(([key, { label, ...itemProps }]) =>
                    [label, <Form.Item key={`${plugin.id}-${key}`} noStyle {...itemProps} name={[plugin.id, key]} />]),
        )
